import { sectionRoles, sectionSpans, spanField, isPaired, type FloorAssembly } from '../sections/index.ts';
...
export function sectionOpenings(request: BuildingRequest, plan: FloorAssembly, height: number, existing: Opening[]): Opening[] {
  ...
  for (const entrance of openings) {
    ...
    const choices = plan.sections.filter(s => s.edge === entrance.edge && (s.technique === 'deep-bay' || s.technique === 'ribbon-bay' || s.technique === 'paired-glass'))
    ...
    entrance.width = section.technique === 'paired-glass' ? 3 : entranceField.width;
    entrance.offset = section.offset + entranceField.offset + (entranceField.width - entrance.width) / 2;
    entrance.leaves = 2;
  ...
  for (const section of plan.sections) {
    if (['frame-pier', 'paired-solid', 'paired-pier'].includes(section.technique)) continue;
...
export function checkSectionOpenings(floor: FloorLayout): void {
  ...
  for (const opening of floor.openings) {
    ...
    const pairedDoor = section.technique === 'paired-glass' && opening.kind === 'door';
    const valid = pairedDoor ? opening.edge === span.edge && opening.width === 3
      && opening.offset >= field.offset && opening.offset + opening.width <= field.offset + field.width
      : opening.edge === span.edge && Math.abs(opening.offset - field.offset) < 1e-7
