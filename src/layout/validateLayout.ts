...
import { PROPORTIONS, clearHeight, isStorefrontFloor, isPodiumFloor, fitPodiumWindow, minEntranceHeight, minWindowHeight, proportionsOf } from '../rules/proportions.ts';
import { onModule } from './module.ts';
import { crossed, edgeU, type Rect } from './obstructions.ts';
import { openingEnvelope } from './openingEnvelope.ts';
import { checkPocketDoor } from './pocketInvariants.ts';
import { acClusterName } from './acUnits.ts';
import { validateMastAssemblies } from './mastAssembly.ts';
import type { FloorLayout, Layout } from './model.ts';
 ...
 * promises.
 */
export function checkInvariants(layout: Layout, obstacles: Map<number, Rect[]>): void {
