// CLI: npm run generate -- <request.json> <outDir> [--seed S] [--embed | --keys-only]
...
function parseArgs(argv: string[]): Args | null {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === '--seed') seed = argv[++i] as string;
    else if (arg.startsWith('--')) return null;
    else positional.push(arg);
  }
  const [requestPath, outDir] = positional;
  if (!requestPath || !outDir || !materialsDir) return null;
  return { requestPath, outDir, mode, materialsDir, baseUrl, seed };
}
...
if (!args) {
  console.error('usage: npm run generate -- <request.json> <outDir> [--seed S] [--embed | --keys-only] [--materials DIR] [--materials-base URI]');
  process.exit(2);
}

try {
  const raw = JSON.parse(readFileSync(args.requestPath, 'utf8')) as { seed?: unknown };
  const given = args.seed ?? (typeof raw.seed === 'string' && raw.seed ? raw.seed : undefined);
  const seed = given ?? randomSeed();
  console.log(`seed ${seed}${given ? '' : ' (generated)'}`);
  mkdirSync(args.outDir, { recursive: true });
  const { glb, blueprint, textures } = await generate({ ...raw, seed }, {
