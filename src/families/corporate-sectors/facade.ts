  ...
  for (const floor of layout.floors.filter(f => f.index > 0 && f.assembly)) {
    ...
    for (let edge = 0; edge < 4; edge++) {
      ...
      if (group.id === 0) {
        ...
        surface.panels(sink, panel, surface.start, surface.end, y, top, 2, 1.5, 0, 0.15);
      ...
      for (const section of floor.assembly!.sections as FamilySection[]) {
        ...
        if (section.id.includes(':recessed-slit:')) {
          surface.solid(sink, panel, u, end, y, top, -CHANNEL_RECESS, -CHANNEL_RECESS - 0.12, true, true);
          continue;
        }
        ...
        surface.solid(sink, panel, u, end, y, top, 0.02, -0.1, true, true);
