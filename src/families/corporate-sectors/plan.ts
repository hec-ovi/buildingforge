...
function windowPair(width: number, height: number, broad = false): WindowField[] {
  return [
    { offset: 0.22, width: width - 0.44, sill: 0.3, height: 0.32, panes: { cols: Math.max(1, Math.round(width / 3)), rows: 1 } },
    { offset: 0.22, width: width - 0.44, sill: broad ? 1.3 : height - 1.18, height: broad ? height - 1.92 : 0.72, panes: { cols: Math.max(1, Math.round(width / 3)), rows: 1 } },
  ];
}
...
function face(floor: number, group: number, localFloor: number, edge: number, length: number, height: number): FamilySection[] {
  ...
  if (group === 0) {
    ...
    const repeats = Math.max(1, Math.floor(usable / 24));
    const unit = usable / (repeats * 8);
    for (let i = 0; i < repeats; i++) {
      out.add('large-panel', unit * 2);
      out.add('recessed-slit', unit * 2, windowPair(unit * 2, height), 0.55);
      out.add('spacer', unit);
      out.add('cassette-solid', unit);
      for (let j = 0; j < 2; j++) out.add('cassette-window', unit, [{ offset: 0.17, width: unit - 0.34, sill: height - 1.55, height: 1.1 }], 0.35);
    }
  } else if (edge === 2) {
    ...
    const units = Math.max(4, Math.floor(usable / 3));
    const unit = usable / units;
    for (let i = 0; i < units; i++) {
      const mechanical = i === units - 1;
      const masked = (localFloor % 4 < 2 && i >= Math.floor(units * 0.55)) || (localFloor % 4 >= 2 && i < Math.floor(units * 0.3));
      out.add(mechanical ? 'mechanical' : masked ? 'mask-panel' : 'side-window', unit, mechanical || masked ? [] : windowPair(unit, height, true), mechanical ? 0.65 : 0.35);
    }
  }
  out.add('end', TRIM);
  return out.values;
...
export function plan(input: FamilyInput): FamilyPlan {
  ...
  return { grid: 0.5, extent: { width, depth }, corners: ['square', 'square', 'square', 'square'], groups,
    floors: input.floorHeights.map((height, floor) => {
      ...
      return { floor, group: group.id, outline: outline.map(p => [...p]), balconySections: [], sections: [width, depth, width, depth].flatMap((length, edge) => face(floor, group.id, floor - group.fromFloor, edge, length, height)) };
    }),
  };
}
