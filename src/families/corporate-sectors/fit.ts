...
export function fit(input: FamilyInput): { outline: Point[]; width: number; depth: number; inset: number } {
  ...
  const width = fittedLength(w - BODY_MARGIN * 2), depth = fittedLength(d - BODY_MARGIN * 2);
  if (Math.min(width, depth) < FIXED_FRONT + WINDOW_CELL) throw new RangeError('Corporate sectors require 17 m body faces and a 3.5 m perimeter for attached details.');
