...
/** The same quarter-circle boundary for the solid podium and glazed upper floors. */
export function cornerPoints(rectangle: Point[]): Point[] {
  ...
  for (let slice = 0; slice < CORNER_SLICES; slice++) {
    ...
    points.push([center[0] + CORNER_RADIUS * (x[0] * Math.cos(angle) + z[0] * Math.sin(angle)),
      center[1] + CORNER_RADIUS * (x[1] * Math.cos(angle) + z[1] * Math.sin(angle))]);
  }
  points.push([c[0] - CORNER_RADIUS * x[0], c[1] - CORNER_RADIUS * x[1]]);
  return points;
}
...
export function glazedCorner(rectangle: Point[], outline: Point[], sections: FamilySection[]): void {
  ...
  for (const [slice, point] of cornerPoints(rectangle).slice(0, -1).entries()) {
