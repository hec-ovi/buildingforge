...
const input: FamilyInput = { rectangle: [[0, 0], [32, 0], [32, 24], [0, 24]],
  floorHeights: [4.4, 3.2, 4.1, 3.5, 4.3, 3.2, 4, 3.7], seed: 'industrial-solid-contract' };
...
describe('industrial-solid public family contract', () => {
  it('fits complete solid sections, with an entrance field and separate windows on every real upper floor', () => {
    ...
    for (const floor of result.floors) for (let edge = 0; edge < 4; edge++) {
      ...
      for (const section of sections) {
        ...
        if (floor.floor && section.technique === 'paired-glass') expect(section.windows!.length).toBeGreaterThanOrEqual(2);
        else expect(section.windows).toEqual([]);
        for (const w of section.windows ?? []) {
          expect(w.offset).toBeGreaterThan(0); expect(w.offset + w.width).toBeLessThan(section.width);
          expect(w.height).toBeLessThanOrEqual(0.62);
          expect(w.width).toBeGreaterThan(w.height * 2);
          expect(w.sill + w.height).toBeLessThan(input.floorHeights[floor.floor]! - 0.6);
        }
      }
  ...
  it('builds broad bands and angular supports within the parcel, preserving window, pocket and connection clearances', () => {
    for (const fixedFaces of [false, true]) {
      ...
      expect(builder.parts.some(p => p.name.includes(':angular-support:') && p.prims.size > 0)).toBe(true);
      expect(builder.parts.some(p => p.name.endsWith(':broad-group-band') && p.prims.size > 0)).toBe(true);
      for (const part of builder.parts) for (const prim of part.prims.values()) {
        ...
        if (!part.name.startsWith(`industrial-solid:${part.floor}:0:`)) continue;
