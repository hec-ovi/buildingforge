...
export const FAMILY_IDS = ['corporate-sectors', 'faceted-bays', 'white-grid', 'balcony-grid', 'mirror-shutters', 'mirror-frame'] as const;
export type FamilyArchitecture = typeof FAMILY_IDS[number];
const families = new Map<string, BuildingFamily>([corporate, faceted, white, balcony, shutters, mirror].map(f => [f.id, f]));
