...
`new SectionAssembler().assemble(input)` returns an assembly; `sectionRoles(section, height)` returns its nine fields. Inputs: [assembly](schemas/input.schema.json), [section and height](types.ts). Outputs: [assembly and role fields](schemas/output.schema.json), [types](types.ts). Dependencies: [building families](../families/CONTRACT.md) for their registered plans.
