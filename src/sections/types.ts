...
export type Architecture = 'rounded-corner' | 'chamfered-corners' | 'terrace-blocks' | 'paired-rounded' | 'paired-rectangular';
export type CornerTechnique = 'square' | 'rounded' | 'chamfered';
export type SectionTechnique = 'deep-bay' | 'ribbon-bay' | 'corner-leg' | 'rounded-glass' | 'chamfered-glass' | 'frame-pier' | 'paired-glass' | 'paired-solid' | 'paired-pier';
...
export interface Section {
  ...
  border: { left?: number; right?: number; side: number; bottom: number; top: number; depth: number };
  corner?: number;
}
