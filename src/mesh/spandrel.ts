...
/** Closed infill sharing the surrounding frame's front and back planes. */
export function meshSpandrel(
  sink: PartSink, basis: FrameBasis, rect: FrameRect,
  front: number, back: number, material: string, uvMode: 'face' | 'exact' = 'face',
): void {
  ...
  [0, (rect.y1 - rect.y0) / 2, 0], [basis.n[0] * depth / 2, 0, basis.n[1] * depth / 2], uvMode);
}
