...
export function meshScenicCurve(sink: PartSink, floor: FloorLayout, opening: Opening, sections: Section[], wallDepth: number,
  details: Pick<ScenicRoomInput, 'lights' | 'state' | 'warm'>): void {
  const curve = curvedRoomFrame(floor, sections, -wallDepth);
  ...
  for (const span of spans) {
    ...
    const p = field.point(span.offset, 0, -wallDepth), q = field.point(span.offset + span.width, 0, -wallDepth);
