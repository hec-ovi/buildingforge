...
import { bandCaps, HOST_GLASS, liningStart, RIBBON, ribbonEdges, shellBody, storeyOf } from './common.ts';
...
function bay(context: PieceContext, cell: Cell, index: number): void {
  ...
  shellBody(context, cell, {
    height, recess: RECESS, mullion: 0.09, liningStart: liningStart(context, index, RECESS),
  ...
  if (band === 'ground') cell.solid(sink, context.material('wall-trim'), SLOT_START, SLOT_END, 0.15, 0.45, 0.1, 0, { back: false });
  if (band === 'crown') {
    // Stepped pier heads finish the group; the slot gets a flat parapet band.
    jointPier(sink, cell, context.material('wall'), { width: PIER, depth: PROJECTION + 0.14, y0: storey, y1: height, caps: { bottom: false } });
    cell.solid(sink, context.material('wall-trim'), SLOT_START, SLOT_END, storey, height - 0.2, 0.12, 0, { back: false, bottom: false });
  }
}
...
function cornerArm(context: PieceContext, cell: Cell, index: number): void {
  ...
  const storey = band === 'crown' ? storeyOf(height, HEAD) : height;
  const sink = context.part('shell');
  shellBody(context, cell, { height, recess: RECESS, liningStart: liningStart(context, index, RECESS) });
  cell.solid(sink, context.material('wall'), 0, cell.length, 0, storey, PROJECTION, 0,
    { ...bandCaps(band), back: false, start: false, end: false });
  if (band === 'crown') {
...
function entranceBay(context: PieceContext, cell: Cell, index: number): void {
  ...
  cell.solid(sink, stone, u0, u0 + 0.6, 0, head - 0.5, PORTAL, 0, { back: false });
  cell.solid(sink, stone, u0 + portalWidth - 0.6, u0 + portalWidth, 0, head - 0.5, PORTAL, 0, { back: false });
  cell.solid(sink, stone, u0, u0 + portalWidth, head - 0.5, head, PORTAL, 0, { back: false });
  cell.solid(sink, context.material('portal-light'), u0 + 0.6, u0 + portalWidth - 0.6, head - 0.62, head - 0.5, PORTAL - 0.04, 0.02, { back: false });
...
export const recipe: KitRecipe = {
  ...
  build(context) {
    ...
    if (context.piece === 'corner') {
      ...
      if (context.band === 'crown') cornerFillet(sink, context.material('wall'), PROJECTION + 0.14, storey, context.height, { bottom: false });
    }
  },
};
