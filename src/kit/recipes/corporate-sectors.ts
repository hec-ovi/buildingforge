...
import { bandCaps, HOST_GLASS, storeyOf } from './common.ts';
 ...
 * between them. The shared backing closes the inner face.
 */
function body(context: PieceContext, cell: Cell, storey: number, openings: Opening[]): void {
...
function panelWings(context: PieceContext, cell: Cell, storey: number): void {
  ...
  cell.solid(sink, panel, JOINT / 2, WING - JOINT / 2, 0, storey, WING_DEPTH, 0, caps);
  cell.solid(sink, panel, cell.length - WING + JOINT / 2, cell.length - JOINT / 2, 0, storey, WING_DEPTH, 0, caps);
}
...
function bay(context: PieceContext, cell: Cell): void {
  ...
  const sink = context.part('shell');
  body(context, cell, height, band === 'ground' ? [] : slits(storey));
  ...
  if (band === 'middle') {
    ...
    cell.solid(sink, context.material('wall-trim'), BANK_START, BANK_END, profile.bottom, profile.top, profile.front, profile.back);
    cell.solid(sink, context.material('light'), BANK_START, BANK_END, profile.bottom - 0.25, profile.bottom, profile.front - 0.04, profile.back);
  }
  if (band === 'crown') {
    ...
    for (let u = 0; u < cell.length - 1e-6; u += WINDOW_CELL) {
      cell.solid(sink, shield, u + JOINT / 2, u + WINDOW_CELL - JOINT / 2, storey, height - 0.2, WING_DEPTH + 0.2, 0);
    }
...
function cornerArm(context: PieceContext, cell: Cell): void {
  ...
  wall(sink, cell, skin, { u0: 0, u1: cell.length, y0: 0, y1: height });
  for (let u = 0; u < cell.length - 1e-6; u += WINDOW_CELL) {
  ...
  if (band === 'crown') {
    for (let u = 0; u < cell.length - 1e-6; u += WINDOW_CELL) {
      cell.solid(sink, context.material('shield'), u + JOINT / 2, u + WINDOW_CELL - JOINT / 2, storey, height - 0.2, WING_DEPTH + 0.2, 0);
    }
    cell.solid(sink, context.material('roof'), 0, cell.length, height - 0.2, height, WING_DEPTH + 0.3, 0,
      { start: false, end: false });
  }
...
function entranceBay(context: PieceContext, cell: Cell): void {
  ...
  cell.solid(sink, context.material('wall-trim'), 1, cell.length - 1, height - 1.1, height - 0.9, CANOPY, 0);
...
export const recipe: KitRecipe = {
  ...
  build(context) {
    ...
    if (context.piece === 'corner') {
      if (context.band === 'ground') cornerFillet(context.part('shell'), context.material('wall-trim'), RIM, context.height - 0.9, context.height - 0.5);
      if (context.band === 'crown') cornerFillet(context.part('shell'), context.material('roof'), WING_DEPTH + 0.3, context.height - 0.2, context.height);
    }
  },
};
