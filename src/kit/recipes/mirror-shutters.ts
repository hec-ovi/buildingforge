...
function bay(context: PieceContext, cell: Cell): void {
  ...
  if (band === 'ground') {
    ...
    for (const u of [0, 2.5, 5]) cell.solid(sink, context.material('ground'), u + 0.05, u + 2.45, 0.4, 2.4, 0.06, 0);
  } else {
  ...
  wall(sink, cell, context.material('wall-trim'), {
    u0: SPINE_START, u1: SPINE_END, y0: 0, y1: storey, openings: windows, facing: -1,
  });
...
function cornerArm(context: PieceContext, cell: Cell): void {
  ...
  cell.solid(sink, context.material('column'), 0, CORNER_PIER, 0, storey, 0.12, 0, { ...bandCaps(band), start: false });
...
function entranceBay(context: PieceContext, cell: Cell): void {
  ...
  for (const u of [SPINE_START - 1.4, SPINE_END + 1.4]) {
    cell.solid(sink, context.material('wall-trim'), u - 0.2, u + 0.2, 0, height - 0.2, 2.1, 0);
...
export const recipe: KitRecipe = {
  ...
  build(context) {
    ...
    if (context.piece === 'corner') {
      const storey = context.band === 'crown' ? context.height - CORNICE : context.height;
      cornerFillet(context.part('shell'), context.material('column'), 0.12, 0, storey, bandCaps(context.band));
