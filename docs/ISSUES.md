...
- Circulation depth: the plan measures the clear depth to the core solid Interior publishes (stair A at the placement its feasibility returns), behind the wall an opening sits in and as the gap across the plate, and gives up any window under 1.20 m on either. The placement moves when Interior reads the room envelopes and the roof, so the plan asks with both in hand.
- Roof access: on a faceted-bays plan of four by three bays the faceted roof plate is about 15 m deep, and most floor counts leave no room for the stair housing plus its 0.9 m walk space, so they publish `bulkhead: null` and no roof access (2, 3, 4, 12 and 36 floors; 6 and 8 hold it). Four by four bays and larger hold it throughout. Either those plans keep no roof access or the walk space shrinks on faceted plates; the choice belongs to Engine and Interior.
