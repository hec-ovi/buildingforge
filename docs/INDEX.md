...
- src/layout: style freeze, massing and the shared core placement every plate keeps (src/layout/core.ts, read from the interior's published constants and stair recipe), floor stack, facade bays, glazing pane grids, facade relief, the per-face obstacle map and its clear-rectangle scan, condenser unit clusters, features. Depends on core and rules.
