...
- **Blueprint JSON** (`schemas/blueprint.schema.json`): per floor, basements included, the index, kind, elevation, height, outline and every opening positioned by outline edge, offset and sill, with curtain state, pane grid, fitted door set, door role and movement clearance, balcony dimensions and material key. Each face publishes its exact panel seams, opening-free wall runs and safe partition anchors. The rest covers signage, screens, lights, facade style, measured wall depth, opaque slab bands, facade equipment, fire escape, roof bulkhead and material keys.
