...
Status: v0.12.0, implemented. Schemas stable to build against; additive fields may come, breaking changes go through the orchestrator.
...
- A window unit is a frame profile standing proud of the wall with real reveal depth, a mullion grid splitting the opening into `panes.cols` x `panes.rows` panes, and the glass recessed a few cm behind the profile. Profile sections and pane limits are seeded once per building, so a facade is consistent and buildings differ.
